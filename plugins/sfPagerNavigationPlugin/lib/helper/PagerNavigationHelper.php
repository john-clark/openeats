...
function my_pager_navigation($pager, $uri, $update)
{
  $navigation = '';
  ...
  {  
    ...
    if ($pager->getPage() != 1)
    {
      $navigation .= link_to_remote(image_tag('/sf/sf_admin/images/first.png', 'align=absmiddle'), array('url'=>$uri.'1', 'update' => $update));
      $navigation .= link_to_remote(image_tag('/sf/sf_admin/images/previous.png', 'align=absmiddle'), array('url' => $uri.$pager->getPreviousPage(), 'update' => $update)).'&nbsp;';
    }
    ...
    foreach ($pager->getLinks() as $page)
    {
        ...
        $links[] = ($page == $pager->getPage()) ? $page : link_to_remote($page, array('url' => $uri.$page, 'update' => $update));
      //$links[] = link_to_unless($page == $pager->getPage(), $page, $uri.$page);
    }
    $navigation .= join(' ', $links);
    ...
    if ($pager->getPage() != $pager->getLastPage())
    {
      $navigation .= '&nbsp;'.link_to_remote(image_tag('/sf/sf_admin/images/next.png', 'align=absmiddle'), array('url' =>$uri.$pager->getNextPage(), 'update' => $update));
      $navigation .= link_to_remote(image_tag('/sf/sf_admin/images/last.png', 'align=absmiddle'), array('url' => $uri.$pager->getLastPage(), 'update' => $update));
    }
